<script setup>
// HeatSafetyDetailView 를 PrimeVue 로 재스킨. 로직 동일, 이모지 제거.
import { ref, computed, onMounted } from 'vue'
import { useRoute } from 'vue-router'
import Card from 'primevue/card'
import Button from 'primevue/button'
import Message from 'primevue/message'
import ProgressSpinner from 'primevue/progressspinner'
import Divider from 'primevue/divider'
import HeatAppHeader from '@/components/exercise-ui/heat-safety/HeatAppHeader.vue'
import HeatStageBadge from '@/components/exercise-ui/heat-safety/HeatStageBadge.vue'
import HeatTimeline from '@/components/exercise-ui/heat-safety/HeatTimeline.vue'
import SafetyChecklist from '@/components/exercise-ui/heat-safety/SafetyChecklist.vue'
import { fetchWorksiteWeather, fetchForecastByCoord } from '@/api/openWeatherApi'
import { computeFeelsLike, getThermalStage } from '@/heat/heatIndex'
import { useWorksiteUiStore } from '@/stores/worksiteUiStore'
import { useFeelsLikeUiStore } from '@/stores/feelsLikeUiStore'
import { useSeasonTheme } from '@/composables/useSeasonTheme'

useSeasonTheme()

const route = useRoute()
const store = useWorksiteUiStore()
const seasonStore = useFeelsLikeUiStore()

const isWinter = computed(() => seasonStore.season === 'winter')
const worksite = computed(() => store.getWorksite(route.params.siteId))
const weather = ref(null)
const forecast = ref([])
const isLoading = ref(false)
const errorMessage = ref('')

const feelsLike = computed(() =>
  weather.value ? computeFeelsLike(seasonStore.season, weather.value) : null,
)
const stage = computed(() => getThermalStage(seasonStore.season, feelsLike.value))

onMounted(async () => {
  if (!worksite.value) {
    errorMessage.value = `등록되지 않은 작업장입니다. (id: ${route.params.siteId})`
    return
  }
  isLoading.value = true
  try {
    weather.value = await fetchWorksiteWeather(worksite.value)
    try {
      forecast.value = await fetchForecastByCoord(worksite.value.lat, worksite.value.lon)
    } catch (forecastError) {
      console.warn('[WeatherUi/HeatSafety] 예보 조회 실패:', forecastError)
    }
  } catch (error) {
    console.error('[WeatherUi/HeatSafety] 상세 조회 실패:', error)
    errorMessage.value = '날씨 데이터를 불러오지 못했습니다.'
  } finally {
    isLoading.value = false
  }
})
</script>

<template>
  <div class="weather-ui-page weather-ui-surface">
    <HeatAppHeader />

    <Message v-if="errorMessage" severity="error" :closable="false">{{ errorMessage }}</Message>
    <div v-else-if="isLoading" class="loading">
      <ProgressSpinner style="width: 40px; height: 40px" stroke-width="4" />
      <span>불러오는 중...</span>
    </div>

    <template v-else-if="worksite">
      <h2 class="page-title">{{ worksite.name }}</h2>

      <Card class="now-card" :style="{ borderInlineStartColor: stage.color }">
        <template #content>
          <div class="now-head">
            <span class="now-feels">
              체감 {{ feelsLike === null ? '산출 불가' : `${feelsLike}°C` }}
            </span>
            <HeatStageBadge :stage="stage" />
          </div>
          <p v-if="weather" class="now-sub">
            기온 {{ weather.temp }}°C · 습도 {{ weather.humidity }}% · 풍속 {{ weather.windSpeed }} m/s
          </p>
          <p class="now-guide">{{ stage.guide }}</p>
        </template>
      </Card>

      <Divider />
      <h3 class="section-title">시간대별 위험 예보</h3>
      <HeatTimeline :forecast="forecast" />

      <Divider />
      <h3 class="section-title">오늘 기본수칙 점검</h3>
      <SafetyChecklist :site-id="worksite.id" />

      <Divider />
      <h3 class="section-title">{{ isWinter ? '한랭질환' : '온열질환' }} 발생 시</h3>
      <ol v-if="isWinter" class="first-aid">
        <li>따뜻한 곳으로 옮기고 젖은 옷을 벗긴 뒤 담요·마른 옷으로 보온합니다.</li>
        <li>의식이 있으면 따뜻한 음료(술·카페인 제외)를 천천히 마시게 합니다.</li>
        <li>의식이 없거나 심한 저체온이면 곧바로 <strong>119</strong>에 신고하고, 몸을 거칠게 문지르지 않습니다.</li>
      </ol>
      <ol v-else class="first-aid">
        <li>즉시 시원한 곳으로 옮기고 옷을 느슨하게 합니다.</li>
        <li>물수건·얼음으로 몸을 식히고, 의식이 있으면 물을 마시게 합니다.</li>
        <li>의식이 없거나 증상이 심하면 곧바로 <strong>119</strong>에 신고합니다. (의식 없을 땐 물 억지로 X)</li>
      </ol>

      <RouterLink to="/weather-ui/heat-safety" custom v-slot="{ navigate }">
        <Button label="현장 현황으로" icon="pi pi-arrow-left" text @click="navigate" />
      </RouterLink>
    </template>
  </div>
</template>

<style scoped>
.page-title {
  font-size: 1.2rem;
  margin: 0 0 12px;
}

.section-title {
  font-size: 1rem;
  margin: 0 0 8px;
}

.now-card {
  border-inline-start: 5px solid var(--p-content-border-color);
  margin-bottom: 8px;
}

.now-head {
  display: flex;
  align-items: center;
  gap: 10px;
}

.now-feels {
  font-size: 1.4rem;
  font-weight: 700;
}

.now-sub {
  margin: 8px 0 4px;
  color: var(--p-text-muted-color);
  font-size: 0.85rem;
}

.now-guide {
  margin: 0;
  font-size: 0.9rem;
}

.first-aid {
  padding-left: 20px;
  font-size: 0.85rem;
  line-height: 1.7;
}

.loading {
  display: flex;
  align-items: center;
  gap: 12px;
  color: var(--p-text-muted-color);
  padding: 12px 0;
}
</style>
