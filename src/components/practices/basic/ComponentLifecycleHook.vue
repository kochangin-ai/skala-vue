...
// reactive 변수 초기화 시점이라 아직 실제 DOM은 존재하지 않는다.
console.log('1. [setup] 컴포넌트가 메모리에 생성되었습니다. (DOM 접근 불가능)')
// 부착 (Mounting) 단계
// 실제 DOM에 렌더링이 끝난 직후 딱 한 번만 실행된다.
onMounted(() => {
...
// DOM이 이미 갱신된 이후에 실행되므로 변경된 화면 값을 그대로 읽을 수 있다.
onUpdated(() => {
  console.log(
...
// 컴포넌트와 함께 정리되지 않는 리소스(타이머, 이벤트 리스너 등)를 여기서 해제해야 한다.
onUnmounted(() => {
