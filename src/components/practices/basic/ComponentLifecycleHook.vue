...
const visible = ref(true) // false가 되면 v-if로 컴포넌트가 파괴됨
const count = ref(0)
...
<template>
  <div>
    ...
    <button @click="visible = false">🔴 실습 컴포넌트 파괴하기 (v-if="false")</button>
    ...
    <p v-else>💀 컴포넌트가 파괴되었습니다. (onUnmounted 실행됨, 콘솔 확인)</p>
  </div>
</template>
